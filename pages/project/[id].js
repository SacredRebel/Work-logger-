...
function ImageGrid({ images, color, projectId, date, onDelete }) {
  ...
  const Group = ({imgs, label, tagBg, allImages, startOffset}) => {
    ...
    return (
      <div>
        ...
        <div style={{display:'grid',gridTemplateColumns:imgs.length===1?'1fr':'1fr 1fr',gap:8}}>
          {imgs.map((img,idx)=>(
            <div key={idx} onClick={()=>setLbIdx(startOffset+idx)}
              ...
              <div style={{position:'absolute',top:7,left:7,background:tagBg,
                backdropFilter:'blur(4px)',color:'#fff',fontSize:9,fontWeight:800,
                letterSpacing:'0.08em',textTransform:'uppercase',padding:'3px 8px',borderRadius:9999}}>
                {label}
              </div>
              {/* Tap indicator */}
              <div style={{position:'absolute',inset:0,background:'transparent',transition:'background 0.15s'}}
                onPointerDown={e=>e.currentTarget.style.background='rgba(0,0,0,0.12)'}
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <Group imgs={before} label="Before" tagBg="rgba(251,191,36,0.85)" allImages={images} startOffset={0}/>
      <Group imgs={after}  label="After"  tagBg="rgba(34,197,94,0.85)"  allImages={images} startOffset={before.length}/>
      {lbIdx !== null && (
        <Lightbox
          images={[...before,...after]}
          startIndex={lbIdx}
...
function EntryCard({ entry, categories, color, projectId, onRefresh }) {
  ...
  return (
    <div style={{
      ...
      border:`1px solid ${open?color+'40':'var(--bdr)'}`,
      boxShadow:open?`0 4px 24px ${color}18,var(--shd)`:'var(--shd)',
      transition:'border-color 0.2s,box-shadow 0.2s',
    }}>
      <div onClick={()=>setOpen(o=>!o)} style={{
        display:'flex',alignItems:'center',gap:12,padding:'15px 16px',
        cursor:'pointer',userSelect:'none',
        background:open?`${color}07`:'transparent',transition:'background 0.2s',
      }}>
        {/* Left accent bar */}
        <div style={{width:4,height:50,borderRadius:2,background:color,flexShrink:0,opacity:open?1:0.45,transition:'opacity 0.2s'}}/>
        {/* Hours badge */}
          ...
          <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
            ...
            {images.length>0 && (
              <span style={{fontSize:11,color:'var(--t4)'}}>
                📷 {images.length}
                {beforeCount>0&&afterCount>0&&` (${beforeCount}B·${afterCount}A)`}
              </span>
            )}
          </div>
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:6,flexShrink:0}}>
          ...
          <div style={{
            width:28,height:28,borderRadius:9,background:'var(--s3)',border:'1px solid var(--bdr)',
            display:'flex',alignItems:'center',justifyContent:'center',
            transition:'transform 0.22s cubic-bezier(0.34,1.56,0.64,1)',
            transform:open?'rotate(90deg)':'none',
          }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" style={{width:13,height:13,color:'var(--t3)'}}>
  ...
  return (
    <div>
      {dates.map((date,di)=>{
        ...
        const allOrdered = [...before,...after];

        const PhotoRow = ({list, label, tagBg, offset}) => {
          if (!list.length) return null;
          return (
            <div style={{marginBottom:12}}>
              <div style={{fontSize:10,fontWeight:800,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:7,
                color:label==='Before'?'#FBBF24':'#22C55E'}}>
                {label} · {list.length}
              </div>
        ...
        return (
          <div key={date}>
            ...
            <PhotoRow list={before} label="Before" tagBg="rgba(251,191,36,0.88)" offset={0}/>
            <PhotoRow list={after}  label="After"  tagBg="rgba(34,197,94,0.88)"  offset={before.length}/>
          </div>
        );
      })}
      ...
      <main className="main">
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:18}}>
          {[
            ...
            <div key={lbl} style={{background:'var(--s1)',borderRadius:14,border:'1px solid var(--bdr)',boxShadow:'var(--shd)',padding:'11px 6px',display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',gap:3}}>
        ...
        {/* Heatmap */}
        <div style={{background:'var(--s1)',borderRadius:18,border:'1px solid var(--bdr)',boxShadow:'var(--shd)',padding:'16px',marginBottom:18}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:14}}>
            <div>
              ...
              <div style={{fontSize:11,color:'var(--t3)'}}>{pEntries.length} days · {weekH>0?`${weekH.toFixed(1)}h this week`:'no logs this week'}</div>
            </div>
          </div>
        ...
        {/* Tabs */}
        <div style={{display:'flex',background:'var(--s2)',borderRadius:14,padding:'3px',gap:2,marginBottom:18,border:'1px solid var(--bdr)'}}>
          {[
            {key:'log',label:`Work Log (${pEntries.length})`},
            {key:'photos',label:`Photos${totalImgs>0?` (${totalImgs})`:''}`},
          ].map(({key,label})=>(
            <button key={key} onClick={()=>setTab(key)} style={{
              flex:1,padding:'8px 0',borderRadius:11,
              fontSize:12,fontWeight:700,fontFamily:'inherit',border:'none',cursor:'pointer',
